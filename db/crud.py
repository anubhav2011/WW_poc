...
def clear_educational_documents_for_reupload(worker_id: str) -> bool:
    """
    Clear educational document data for re-upload after verification mismatch.
    
    Use case: When personal and educational name/dob don't match,
    user chooses to re-upload ONLY educational document.
    
    This clears:
    - educational_documents row (all columns set to NULL or deleted)
    - verification_status set back to "pending"
    - personal document data is PRESERVED
    
    Args:
        worker_id: Worker ID to clear educational documents for
    ...
    try:
        ...
        logger.info(f"[REUPLOAD] Clearing educational documents for worker {worker_id}...")
        
        # Delete entire row from educational_documents
        cursor.execute("DELETE FROM educational_documents WHERE worker_id = ?", (worker_id,))
        deleted_count = cursor.rowcount
        
        if deleted_count > 0:
            logger.info(f"[REUPLOAD] Deleted {deleted_count} educational document row(s)")
        else:
            logger.info(f"[REUPLOAD] No educational documents found to delete")
        
        # Reset verification status in workers table
        cursor.execute("""
            UPDATE workers 
            SET education = NULL, 
                has_education = 0,
        ...
        conn.commit()
        logger.info(f"[REUPLOAD] ✓ Educational documents cleared for {worker_id}. Ready for re-upload.")
        return True
        
    except Exception as e:
        logger.error(f"[REUPLOAD] ✗ Error clearing educational documents for {worker_id}: {str(e)}", exc_info=True)
        return False
    finally:
        if conn is not None:
            conn.close()
...
def clear_all_documents_for_reupload(worker_id: str) -> bool:
    """
    Clear ALL document data for complete re-upload after verification mismatch.
    
    Use case: When personal and educational name/dob don't match,
    user chooses to re-upload PERSONAL document (which means start fresh).
    
    This clears:
    - personal_documents row (complete reset)
    - educational_documents row (complete reset)
    - work_experience data
    - voice_sessions related to this worker
    - All verification flags
    
    The worker profile is essentially reset to initial state.
    
    Args:
        worker_id: Worker ID to clear all documents for
    ...
    try:
        ...
        logger.info(f"[REUPLOAD] Performing FULL DATA CLEAR for worker {worker_id}...")
        
        # Delete personal documents
        cursor.execute("DELETE FROM personal_documents WHERE worker_id = ?", (worker_id,))
        personal_deleted = cursor.rowcount
        logger.info(f"[REUPLOAD] Deleted {personal_deleted} personal document row(s)")
        
        # Delete educational documents
        cursor.execute("DELETE FROM educational_documents WHERE worker_id = ?", (worker_id,))
        education_deleted = cursor.rowcount
        logger.info(f"[REUPLOAD] Deleted {education_deleted} educational document row(s)")
        ...
        logger.info(f"[REUPLOAD] Deleted {experience_deleted} work experience row(s)")
        
        # Delete voice sessions
        cursor.execute("DELETE FROM voice_sessions WHERE worker_id = ?", (worker_id,))
        voice_deleted = cursor.rowcount
        logger.info(f"[REUPLOAD] Deleted {voice_deleted} voice session row(s)")
        
        # Reset workers table to initial state
        cursor.execute("""
            UPDATE workers 
            SET personal_extracted_name = NULL,
        ...
        conn.commit()
        logger.info(f"[REUPLOAD] ✓ ALL documents and experience cleared for {worker_id}. Complete reset ready for new uploads.")
        return True
        
    except Exception as e:
        logger.error(f"[REUPLOAD] ✗ Error clearing all documents for {worker_id}: {str(e)}", exc_info=True)
        return False
    finally:
        if conn is not None:
            conn.close()
